...
1. **FR/NFR** — requirement ID from `docs/FUNCTIONAL_REQUIREMENTS.md`.
2. **Spec** — manifest schema, validation contract, and non-regression constraints.
3. **Docs** — operator/user documentation and rich media placeholders.
4. **Code** — CLI, HTTP API, UI components (`ShotGallery`, `RecordingEmbed`, `JourneyViewer`), schema validator, OCR assertion, or recording harness.
...
![phenotype-journey CLI commands — record, verify, assert, sync output, and exit code evidence](../assets/rich-media/phenotype-journeys/phenotype-journey-cli-commands.gif)
...
![phenotype-journey manifest schema — valid bundle, invalid bundle, error report, and CI verdict](../assets/rich-media/phenotype-journeys/manifest-schema-validation.png)
...
![phenotype-journey embed rendering — ShotGallery, RecordingEmbed, accessibility state, and eval verdict](../assets/rich-media/phenotype-journeys/shot-gallery-recording-embed.png)
...
![phenotype-journey auth — denied access, scoped access, audit log, and eval verdict](../assets/rich-media/phenotype-journeys/authn-authz-least-privilege.png)
