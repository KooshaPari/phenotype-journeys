    ...
    async fn show_tui_aware_preview(
        ...
        match tui_config.preview_method {
            ...
            TuiPreviewMethod::SeparatePane => {
                ...
                println!("≡ƒû╝∩╕Å  Image detected: {}", detected_image.path.display());
            ...
            TuiPreviewMethod::External => {
                ...
                println!("≡ƒû╝∩╕Å  Image detected: {} (use external viewer)", detected_image.path.display());
...
impl LivePreviewSystem {
    ...
    async fn show_floating_preview(&self, path: &Path) -> Result<()> {
        ...
        print!("≡ƒû╝∩╕Å  Live Preview: {}", path.file_name().unwrap_or_default().to_string_lossy());
