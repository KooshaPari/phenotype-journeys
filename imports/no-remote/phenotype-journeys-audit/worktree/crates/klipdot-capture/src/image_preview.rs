...
impl ImagePreviewManager {
    ...
    pub async fn show_compact_preview(&self, image_path: &Path) -> Result<String> {
        ...
        let mut info = format!("≡ƒû╝∩╕Å {}", file_name);
    ...
    async fn show_external_preview(&self, viewer: &str, image_path: &Path, max_width: Option<u32>, max_height: Option<u32>) -> Result<()> {
        ...
        match viewer {
            ...
            "qlmanage" => {
                ...
                println!("≡ƒû╝∩╕Å  Opening with QuickLook: {}", image_path.file_name().unwrap_or_default().to_string_lossy());
            ...
            "open" => {
                ...
                println!("≡ƒû╝∩╕Å  Opening with default app: {}", image_path.file_name().unwrap_or_default().to_string_lossy());
    ...
    async fn show_text_info(&self, image_path: &Path) -> Result<()> {
        ...
        println!("≡ƒô╕ Image: {}", file_name);
        println!("≡ƒôÅ Size: {}", file_size);
        if !dimensions.is_empty() {
            println!("≡ƒû╝∩╕Å  Dimensions: {}", dimensions);
        }
        println!("≡ƒôü Path: {}", image_path.display());
        
        // On macOS, offer to open with QuickLook
        if cfg!(target_os = "macos") {
            println!("≡ƒÆí Tip: Run 'qlmanage -p \"{}\"' to preview with QuickLook", image_path.display());
            println!("≡ƒÆí Or: 'open \"{}\"' to open with default app", image_path.display());
        }
    ...
    fn parse_file_dimensions(file_output: &str) -> Option<String> {
        ...
        let re = regex::Regex::new(r"(\d+)\s*[x├ù]\s*(\d+)").ok()?;
