...
//! docs root. The test does not shell out to the binary ΓÇö instead it
...
//!
//! Traces to: phenodocs PR #168, journey-traceability-standard ┬ºCI Gate.
...
fn verify_subcommand_passes_bootstrap_fixture() {
    ...
    // 1. verify_manifest (Claude-describe+judge mock) ΓÇö must succeed offline.
    ...
    // 2. Assertion engine against the docs root ΓÇö must report zero violations.
