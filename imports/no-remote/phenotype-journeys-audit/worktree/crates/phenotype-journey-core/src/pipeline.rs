...
//!
//! * `record_all`           ΓåÉ `record-all.sh`
//! * `extract_keyframes`    ΓåÉ `extract-keyframes.sh`
//! * `verify_all`           ΓåÉ `verify-manifests.sh` + `mock-anthropic-server.py`
//! * `sync_artefacts`       ΓåÉ `sync-cli-journeys.sh`, `sync-journey-artefacts.sh`,
//!                             `sync-streamlit-journeys.sh`, `sync-adrs.sh`,
//!                             `sync-research.sh`
//!
//! Everything here is domain-agnostic ΓÇö hwLedger just passes absolute paths
...
pub struct RecordOptions {
    ...
    /// Parallel tapes (default 1 ΓÇö VHS serialises on PTY).
    pub parallel: usize,
...
pub fn record_all(opts: &RecordOptions) -> Result<RecordSummary, JourneyError> {
    ...
    for tape in &tapes {
        ...
        if !content.contains("__EXIT_") {
            eprintln!(
                "warn: {} does not emit an __EXIT_$?__ sentinel ΓÇö expected_exit \
...
pub fn extract_keyframes(opts: &ExtractOptions) -> Result<Vec<ExtractResult>, JourneyError> {
    ...
    for mp4 in &mp4s {
        ...
        // CRITICAL: always clear stale frames ΓÇö a shorter re-recording would
...
fn verify_one(manifest_path: &Path, opts: &VerifyOptions) -> Result<VerifyJourneyResult, JourneyError> {
    ...
    // author's `intent` ΓÇö it only looks at the keyframe. In mock mode we
    ...
    for step in manifest.steps.iter_mut() {
        ...
        // (≡ƒƒó/≡ƒƒí/≡ƒö┤ + overlap%/raw score) and the diff panel without
...
pub(crate) fn synthesize_blind_description(slug: &str, index: u32) -> String {
    const TEMPLATES: &[&str] = &[
        ...
        "Shell prompt displaying the result of a subcommand ΓÇö multi-line text output.",
    ];
...
mod tests {
    ...
    fn extract_keyframes_cleans_stale_frames() {
        ...
        // contract ΓÇö exercise them directly.
        let dir = tmp("stale");
        let kf = dir.join("kf");
...
"#,
    ...
    fn verify_collects_violations_from_report() {
        // Exercise the report ΓåÆ verification.assertion_violations union path.
