...
//! `tesseract` is not installed ΓÇö silent skip is not allowed.
...
///
/// * `tesseract` (default) ΓÇö shell out to `tesseract` (or honour
///   [`OCR_CMD_ENV`] if set).
/// * `vision` ΓÇö Apple Vision `VNRecognizeTextRequest` (macOS only, requires
...
///
/// * `manifest_path` ΓÇô path to the `manifest.json` (used only to derive the
///   journey id ΓÇö the manifest content is read separately so callers can also
///   pass an already-materialised [`Manifest`] via [`run_on_manifest`]).
/// * `artefacts_root` ΓÇô directory containing the `keyframes/<journey>/` tree.
...
pub fn run_on_manifest(
    ...
    // trailing underscores (collapsing `__` ΓåÆ `_` or dropping one side), so we
...
/// 2. `PHENOTYPE_JOURNEY_OCR_BACKEND=vision` ΓÇö Apple Vision `VNRecognizeTextRequest`
///    (macOS only; requires the `vision` cargo feature). Fails loudly when
///    the backend is unavailable ΓÇö no silent fallback.
