//! Intent Γåö blind-description agreement scoring.
//!
...
//! |----------------------|----------|-----------------------------------------|
//! | `Jaccard`            | text-text (stemmed token overlap)        | ΓëÑ0.6 / 0.3ΓÇô0.6 / <0.3 |
//! | `SentenceTransformer`| text-text (sentence-transformers cosine) | ΓëÑ0.75 / 0.5ΓÇô0.75 / <0.5 |
//! | `SigLip`             | image-text (SigLIP logits-per-image)     | ΓëÑ0.3 / 0.1ΓÇô0.3 / <0.1 |
//!
//! The three thresholds are intentionally different: Jaccard is punishingly
//! literal (paraphrase ΓåÆ Red), sentence-transformers land paraphrases in the
...
impl AgreementScorer for SigLipScorer {
    ...
    fn score(&self, intent: &str, blind: &str, image: Option<&Path>) -> AgreementReport {
        let img = match image {
            ...
            _ => {
                // No image ΓåÆ no signal for SigLIP; fall back to Jaccard.
                let mut fb = JaccardScorer.score(intent, blind, None);
