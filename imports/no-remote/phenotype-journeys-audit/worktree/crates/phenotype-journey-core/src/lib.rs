...
pub struct Step {
    ...
    /// Intent Γåö blind-description agreement report, populated during
    /// `verify` and consumed by the journey viewer's status chip + diff
    /// panel (Green ΓëÑ0.6, Yellow 0.3ΓÇô0.6, Red <0.3).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// does not require network access ΓÇö this mirrors the behaviour of the
