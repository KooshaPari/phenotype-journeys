...
1. The phenotype-journey engine (`phenotype-journey-core`) runs a journey and
   ...
   - `steps[].intent` ΓÇö what the step intends
   - `steps[].description` ΓÇö Claude's blind description of the screenshot
   - `steps[].screenshot_path` ΓÇö relative path to the keyframe PNG
   - `steps[].annotations[]` ΓÇö bbox highlight/cursor rects
