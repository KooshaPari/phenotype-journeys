...
1. **Frame-numbered screenshot naming convention** ΓÇö dino uses
   ...
   similar `frame-NNN.png` convention, so no code was copied ΓÇö the alignment
   is documented here for consistency.
2. **Capture-driver pattern** ΓÇö `dino/capture_ui_states.py` drives the
