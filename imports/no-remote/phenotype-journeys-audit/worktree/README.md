...
> ΓÜá∩╕Å **AI-Agent-Only Repository**
>
...
Journey harness code kept accreting inside individual projects ΓÇö VHS tapes +
...
- **Canonical manifest schema** (`schema/manifest.schema.json`) ΓÇö source of
  truth for step/verification shape.
- **`phenotype-journey-core`** (Rust) ΓÇö serde types, schema export, and the
  verify loop in both `mock` and `live` (Anthropic API) modes.
- **`phenotype-journey`** (Rust CLI) ΓÇö `record`, `verify`, `validate`, `sync`.
- **`@phenotype/journey-viewer`** (Vue 3) ΓÇö `JourneyViewer` +
  `RecordingEmbed` components for VitePress docs.
- **`@phenotype/journey-playwright`** (TypeScript) ΓÇö script a web page and
...
- **hwLedger** ΓÇö swap `apps/cli-journeys/scripts/verify-manifests.sh` +
  in-theme `JourneyViewer.vue` for this package. (See `MIGRATION.md` in the
  hwLedger repo.)
- **AgilePlus** ΓÇö journeys for feature specs (one per tagged FR).
- **thegent** ΓÇö journeys for plugin onboarding flows.
...
- `tesseract` CLI ΓÇö install with `brew install tesseract` (macOS) or
...
- `must_contain` ΓÇö every listed substring must appear in the OCR of that
  step's keyframe.
- `must_not_contain` ΓÇö none of the listed substrings may appear.
- `expected_exit` ΓÇö the LAST keyframe of the journey must include the
  sentinel `__EXIT_<N>__`.
- `ocr_required` ΓÇö reserved for future "OCR must succeed" gating; default
...
ΓÇö making the gate a hard fail for the build. The legacy positional
