...
rendering:
  ...
  - cli       -> reconstructed rows├ùcols terminal buffer w/ cursor marker
...
Consumers (hwLedger docs-site vendor/phenotype-journeys/ΓÇª-0.1.0.tgz)
...
- Feat(journey-core+viewer): intentΓåöblind agreement scoring + chip/diff UI
...
Green ΓëÑ0.6 / Yellow 0.3ΓÇô0.6 / Red <0.3 buckets. AgreementReport
...
KeyframeLightbox + KeyframeGallery render a ≡ƒƒó/≡ƒƒí/≡ƒö┤ "{overlap}%
...
3. Intent / Blind attribution glyphs: the chips now carry a Γ£ì∩╕Ä glyph
   (Intent = human authoring from intents.yaml) and Γùë glyph
...
as <noscript> fallback. Adds 'Open MP4 Γåù' / 'Open GIF Γåù' raw-asset links below.
...
scrolls within 6rem; 'Show more Γû╛' expands to 20rem; aria-expanded wired.
...
* New `vision` cargo feature on phenotype-journey-core and phenotype-journey
  ...
  bindings ΓÇö no hand-rolled msg_send!.
* Runtime selection via PHENOTYPE_JOURNEY_OCR_BACKEND env var:
  `tesseract` (default), `vision` (Vision), unknown ΓåÆ hard error.
...
`phenotype-journey` CLI owns the full record ΓåÆ extract ΓåÆ verify ΓåÆ sync flow
...
The layer fails loudly when tesseract is missing ΓÇö no silent skip ΓÇö and
...
- Test(smoke): seed minimal smoke test ΓÇö proves harness works (`204e5aa`)
...
- Annotate IntentsFile.journey and IntentStep.intent with `// kept:` ΓÇö
