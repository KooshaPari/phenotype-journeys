 ...
 * title: Streamlit ΓÇö HuggingFace model search journey
...
Run with `bunx playwright test` ΓÇö the fixture reads frontmatter from
`testInfo.file`, runs the test body, and writes the manifest on teardown.

## Example 2 ΓÇö Custom output root
...
## Example 3 ΓÇö Disabling capture per-story

Set `record: false` in the frontmatter to turn a spec into a pure
functional test ΓÇö the manifest is still emitted (with 0 keyframes) so
