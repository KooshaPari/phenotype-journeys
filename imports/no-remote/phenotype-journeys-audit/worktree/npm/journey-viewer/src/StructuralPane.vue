  ...
  <div class="kf-structural-pane" role="region" aria-label="Structural snapshot">
    <div class="kf-structural-header">
      <span class="kf-structural-title">
        <span class="kf-structural-glyph" aria-hidden="true">Γù½</span>
        Structural ΓÇö {{ familyLabel }}
      </span>
      <span class="kf-structural-path" :title="path">{{ shortPath }}</span>
    </div>

    <div v-if="loading" class="kf-structural-muted">loadingΓÇª</div>
    ...
    <template v-else-if="snapshot">
      ...
      <pre v-if="snapshot.family === 'cli'" class="kf-term-buffer" :aria-label="`${snapshot.rows}├ù${snapshot.cols} terminal buffer`">{{ termText }}</pre>
...
const shortPath = computed(() => {
  ...
  const p = props.path
  return p.length > 48 ? 'ΓÇª' + p.slice(-46) : p
})
...
/** Render the CLI cells into a rows├ùcols monospace grid. */
const termText = computed(() => {
  ...
  // Overlay cursor as `Γûê`-style marker if visible and in-bounds.
  if (s.cursor.visible && s.cursor.row < s.rows && s.cursor.col < s.cols) {
    buf[s.cursor.row][s.cursor.col] = 'Γûë'
  }
