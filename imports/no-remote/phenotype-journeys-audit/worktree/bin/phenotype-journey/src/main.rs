...
enum Cmd {
    ...
    Record {
        ...
        only: Option<String>,
        /// Max concurrent tapes (default 1 ΓÇö VHS serialises on PTY).
    ...
    ExtractKeyframes {
        ...
        #[arg(long)]
        tape: Option<String>,
        /// Fallback threshold ΓÇö if fewer I-frames than this are found, switch
        /// to 1 fps sampling.
...
fn cmd_record(tape: PathBuf, out: PathBuf) -> Result<()> {
    ...
    let status = std::process::Command::new("vhs")
        ...
        .status()
        .with_context(|| "failed to invoke `vhs` ΓÇö install charmbracelet/vhs")?;
...
fn cmd_verify_with_root(
    ...
    if let Some(docs) = docs_root {
        ...
        // directly ΓÇö phenodocs layouts co-locate keyframes under
...
/// if any are missing. Strict by design ΓÇö not gated by any env var.
fn cmd_check_verified(roots: Vec<PathBuf>) -> Result<()> {
    ...
    eprintln!(
        "check-verified: FAIL ΓÇö {} of {} manifest(s) lack a sibling manifest.verified.json:",
        unverified.len(),
        manifests.len()
    );
    for m in &unverified {
...
fn cmd_assert(
    ...
    if intents_path.exists() {
        ...
        eprintln!(
            "warn: no intents YAML at {} ΓÇö continuing with manifest-embedded assertions only",
    ...
    if report.no_assertions {
        eprintln!(
            "warn: journey '{}' has zero steps with assertions ΓÇö consider adding must_contain / must_not_contain",
            report.journey_id
        );
    }
...
fn tesseract_annotate(
    ...
    let out = std::process::Command::new("tesseract")
        ...
        .arg("tsv")
        .output()
        .with_context(|| "failed to invoke `tesseract` ΓÇö install with `brew install tesseract`")?;
...
/// Pure-function TSV parser (exposed for tests ΓÇö private helper).
