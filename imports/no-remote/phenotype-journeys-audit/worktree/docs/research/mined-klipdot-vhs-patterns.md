# Mined patterns: KlipDot ΓåÆ phenotype-journeys
...
3. Prefer **authentic command output** (real binary) over mocked terminal text ΓÇö KlipDot `demo-authentic-usage.tape` pattern.
...
KlipDot `.github/workflows/quality-gate.yml` builds `phenotype-tooling` `quality-gate` if missing ΓÇö same pattern as other Phenotype repos. No change required here; documented for parity.
...
- Agent HTTP API and interceptor config ΓÇö see [KommandLineAutomation mined patterns](https://github.com/KooshaPari/KommandLineAutomation/blob/master/docs/research/mined-klipdot-agent-api-patterns.md).

## Related fork-lane repos

| Repo | Role |
|------|------|
| KommandLineAutomation | CLI recording harness ΓÇö pair VHS keyframes with KLA session artifacts |
| KDesktopVirt | Full desktop evidence when flows leave the terminal |

## Provenance

Read-only mine of [KlipDot](https://github.com/KooshaPari/KlipDot) on 2026-05-31. Companion extractions: phenotype-journeys (this doc), KommandLineAutomation (agent API + interceptor config), Benchora/Tracera (kwality lineage ΓÇö separate PRs).
