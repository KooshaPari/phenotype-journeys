...
impl Default for ClipboardToolsConfig {
    fn default() -> Self {
        let mut wayland_tools = crate::WAYLAND_CLIPBOARD_TOOLS
            .iter()
            .map(|s| s.to_string())
            .collect::<Vec<_>>();
        let mut x11_tools = crate::X11_CLIPBOARD_TOOLS
            .iter()
            .map(|s| s.to_string())
            .collect::<Vec<_>>();
...
impl Default for ScreenshotToolsConfig {
    fn default() -> Self {
        ...
        let mut wayland_tools = crate::WAYLAND_SCREENSHOT_TOOLS
            .iter()
            .map(|s| s.to_string())
            .collect::<Vec<_>>();
        let mut x11_tools = crate::X11_SCREENSHOT_TOOLS
            .iter()
            .map(|s| s.to_string())
            .collect::<Vec<_>>();
