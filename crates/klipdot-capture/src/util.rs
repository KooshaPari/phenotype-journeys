...
pub fn get_app_dir() -> super::error::Result<std::path::PathBuf> {
    let app_dir = dirs::data_dir()
        .ok_or_else(|| super::error::Error::Config("Failed to get data directory".to_string()))?
        .join(APP_NAME);
...
pub fn get_config_dir() -> super::error::Result<std::path::PathBuf> {
    let config_dir = dirs::config_dir()
        .ok_or_else(|| super::error::Error::Config("Failed to get config directory".to_string()))?
        .join(APP_NAME);
...
pub fn get_home_dir() -> super::error::Result<std::path::PathBuf> {
    let home_dir = dirs::home_dir()
        .ok_or_else(|| super::error::Error::Config("Failed to get home directory".to_string()))?
...
mod tests {
    ...
    fn test_display_server_detection() {
        ...
        // DisplayServer::MacOS is the expected result on macOS hosts.
        assert!(matches!(
            server,
            DisplayServer::X11
                | DisplayServer::Wayland
