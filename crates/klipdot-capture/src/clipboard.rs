...
impl ClipboardMonitor {
    ...
    fn is_image_data(&self, content: &str) -> bool {
        ...
        {
            if let Ok(data) = base64::engine::general_purpose::STANDARD.decode(content) {
...
// `base64::decode` / `base64::encode` calls in this file refer to the external
// `base64` crate (re-exported via `use base64::Engine` above). An earlier draft
// of this module defined local wrapper mods; those wrappers were never invoked
// and have been removed to keep clippy's `dead_code` gate green.
